...
async function handleWsRequest(request, currentFallbackAddress = null) {
  ...
  // 在这里添加日志，确认 WebSocket 请求被接受
  console.log('🟢 WebSocket 连接已建立，正在处理请求...');
  ...
  // 检查并选择当前的备用地址
  let fbAddr = currentFallbackAddress || fallbackAddress;
  if (!fbAddr && currentWorkerRegion) {
    const bestBackupIP = await getBestBackupIP(currentWorkerRegion);
    if (bestBackupIP) fbAddr = bestBackupIP.domain + ':' + bestBackupIP.port;
  }
  console.log('⚙️ 备用地址：', fbAddr);
  ...
  readable.pipeTo(new WritableStream({
    async write(chunk) {
      console.log('🟡 收到数据块，开始处理...');
      
      if (isDnsQuery) {
        console.log('🔄 处理 DNS 请求...');
      ...
      if (isDnsQuery) {
        console.log('🔄 转发 DNS 请求...');
        return forwardUDP(rawData, serverSock, respHeader);
      }

      // 处理 TCP 连接
      console.log('🔄 正在连接到远程服务器:', hostname, port);
...
async function forwardTCP(addrType, host, portNum, rawData, ws, respHeader, remoteConnWrapper, fbAddr = null) {
  ...
  async function retryConnection() {
    ...
    console.log('🔄 重新尝试连接到备用地址:', fallbackHost, fallbackPort);
...
function closeSocketQuietly(socket) {
  try { 
    ...
    console.error("Close socket error:", error);
  }
}
